...
public class SchoolClass extends RepresentationModel<SchoolClass> implements Serializable
{
	...
	public double avgStudentsHeight1()
	{
		return this.students.stream()
						    .filter(student -> student.getHeight() != null && student.getHeight() > 0)
...
//	}
	...
	{
		return this.students.stream()
							.mapToLong(student -> student.countGrades(grade))
							.sum();
	}
	...
	public long countSubjectGrade(int subjectId,
								  int grade)
	{
		return this.getStudentSubjects().stream()
										.filter(ss -> ss.getSubject().getSubjectId() == subjectId && ss.getGrade() == grade)
										.count();
	}
	//endregion
